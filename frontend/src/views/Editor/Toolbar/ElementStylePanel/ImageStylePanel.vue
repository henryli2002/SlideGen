<template>
  <div class="image-style-panel">
    ...
    <Button class="full-width-btn" @click="aiReplaceVisible = true"><i-icon-park-outline:magic /> AI 替换图片</Button>
    ...
    <Modal v-model:visible="aiReplaceVisible" :width="460">
      <AIImageDialog
        mode="replace"
        @confirm="url => handleAIReplace(url)"
        @close="aiReplaceVisible = false"
      />
    </Modal>
  </div>
</template>
...
import AIImageDialog from '@/views/Editor/AIImageDialog.vue'
...
const aiReplaceVisible = ref(false)
...
// AI 替换图片（保持当前位置和尺寸）
const handleAIReplace = (url: string) => {
  aiReplaceVisible.value = false
  updateImage({ src: url })
}
