  ...
  <div class="aippt-dialog">
    ...
    <template v-if="step === 'setup'">
      ...
      <div class="configs">
        ...
        <div class="config-item">
          <div class="label">RAG检索增强：</div>
          <Switch class="config-content" v-model:value="enableSearch" />
        </div>
        <div class="config-item">
          <div class="label">AI生图：</div>
          <Switch class="config-content" v-model:value="enableImage" />
        </div>
      </div>
    ...
    <div class="preview" v-if="step === 'outline'">
      <div class="outline-loading" v-if="outlineCreating">
        <div class="loading-tip">AI 正在生成大纲，请稍等 ...</div>
      </div>
    ...
    try {
      if (chunk.startsWith('data: ')) {
        ...
        if (payload.status === 'generating' && payload.slide) {
          AIPPT(templateSlides, [payload.slide], undefined, templateTheme.themeColors?.[0], enableImage.value)
        ...
        if (text) {
          ...
          AIPPT(templateSlides, [slide], undefined, templateTheme.themeColors?.[0], enableImage.value)
        }
      }
    }
    catch (err) {
